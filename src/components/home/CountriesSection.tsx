      ...
      <div className="relative group">
        ...
        <div className="flex animate-marquee group-hover:[animation-play-state:paused]" style={{ animationDuration: "24s" }}>
          {doubled.map((country, i) => (
            <div key={`${country.name}-${i}`} className="flex-shrink-0 flex flex-col items-center gap-2 mx-2.5 px-4 py-3 bg-muted/30 border border-border rounded-xl min-w-[106px] hover:border-primary/30 hover:-translate-y-1 transition-all cursor-default">
              <img src={country.flag} alt={country.name} className="w-[62px] h-10 object-cover rounded shadow-md" />
