...
const partners = [
  { name: "Duchess NL", lead: "Dr. Victoria Akai IIPM", role: "CEO", descKey: "implPartners.duchess.desc", icon: Crown, accent: "bg-primary/10 text-primary", logo: duchessLogo, slug: "duchess-nl" },
  { name: "Borderless Trade & Investment", lead: "Dr. Olori Boye-Ajayi", role: "Managing Partner", descKey: "implPartners.borderless.desc", icon: Globe, accent: "bg-ecowas-blue/10 text-ecowas-blue", logo: borderlessLogo, slug: "borderless-trade" },
  { name: "CMD Tourism & Trade Enterprises", lead: "Madam Cecile Mambo Doumbe", role: "CEO", descKey: "implPartners.cmd.desc", icon: Sparkles, accent: "bg-secondary/10 text-secondary", logo: cmdLogo, slug: "cmd-tourism" },
];
              ...
              <Link to={`/partners/${partner.slug}`} className="block h-full">
                <div className="bg-card border border-border rounded-2xl p-6 hover:border-primary/30 transition-all hover:shadow-lg group h-full">
                  <div className="flex flex-col items-start gap-4">
                    <img src={partner.logo} alt={partner.name} className="h-10 w-auto object-contain" loading="lazy" />
                    <div className="flex-1 min-w-0">
                      <h3 className="text-base font-bold text-card-foreground">{partner.name}</h3>
                      <span className="inline-block text-[10px] font-bold uppercase tracking-wider text-primary bg-primary/10 px-2 py-0.5 rounded-full mt-1 mb-2">{partner.role}</span>
                      <p className="text-sm text-muted-foreground leading-relaxed mb-3">{t(partner.descKey)}</p>
