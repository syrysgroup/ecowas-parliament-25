...
const BrandingSettings = () => {
  ...
  const handleUpload = async (file: File, type: "logo" | "favicon") => {
    ...
    try {
      ...
      if (type === "logo") {
        ...
        await supabase.from("site_settings").upsert({ key: "site_logo_url", value: publicUrl }, { onConflict: "key" });
        qc.invalidateQueries({ queryKey: ["site-settings"] });
       } else {
        setFaviconPreview(publicUrl);
        await updateSetting("branding", { ...branding, favicon_url: publicUrl });
        await supabase.from("site_settings").upsert({ key: "site_favicon_url", value: publicUrl }, { onConflict: "key" });
        qc.invalidateQueries({ queryKey: ["site-settings"] });
  ...
  const handleSave = async () => {
    ...
    await supabase.from("site_settings").upsert({ key: "site_logo_url", value: logoPreview }, { onConflict: "key" });
    await supabase.from("site_settings").upsert({ key: "site_name", value: name }, { onConflict: "key" });
    await supabase.from("site_settings").upsert({ key: "site_favicon_url", value: faviconPreview }, { onConflict: "key" });
    qc.invalidateQueries({ queryKey: ["site-settings"] });
